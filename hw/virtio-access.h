...
static inline uint16_t virtio_lduw_phys(VirtIODevice *vdev, hwaddr pa)
{
    if (virtio_access_is_big_endian(vdev)) {
        return lduw_be_phys(pa);
    }
    return lduw_le_phys(pa);
}

static inline uint32_t virtio_ldl_phys(VirtIODevice *vdev, hwaddr pa)
{
    if (virtio_access_is_big_endian(vdev)) {
        return ldl_be_phys(pa);
    }
    return ldl_le_phys(pa);
}

static inline uint64_t virtio_ldq_phys(VirtIODevice *vdev, hwaddr pa)
{
    if (virtio_access_is_big_endian(vdev)) {
        return ldq_be_phys(pa);
    }
    return ldq_le_phys(pa);
}
...
{
    if (virtio_access_is_big_endian(vdev)) {
        stw_be_phys(pa, value);
    } else {
        stw_le_phys(pa, value);
    }
}
...
{
    if (virtio_access_is_big_endian(vdev)) {
        stl_be_phys(pa, value);
    } else {
        stl_le_phys(pa, value);
    }
}
