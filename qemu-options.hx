...
``-device arm-smmuv3,primary-bus=id[,option=...]``
    ...
    The following options are available when accel=on. 'auto' mode is
    supported and, if they are set to 'auto', the value is
    ...
    ``oas=val|auto`` (supported values are 44 and 48. default: 44)
    ...
    ``ssidsize=val|auto`` (val between 0 and 20. default: 0)
